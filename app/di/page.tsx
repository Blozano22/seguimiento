      ...
      <main className="max-w-5xl mx-auto px-4 py-6">
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
            <div className={`grid gap-0 text-xs font-semibold text-gray-500 uppercase px-5 py-3 border-b border-gray-100 bg-gray-50 ${
              activeTab === 'pendientes' ? 'grid-cols-[1fr_1fr_2fr_auto]' : 'grid-cols-[1fr_1fr_2fr_100px]'
            }`}>
              <span>Nivel</span>
              <span>Programa</span>
              <span>Asignatura</span>
              <span>{activeTab === 'pendientes' ? 'Acciones' : 'Estado'}</span>
            </div>
            {currentList.map((c, i) => {
              const estado = String(c.Estado ?? '').trim();
              return (
                <div key={i} className={`grid gap-0 items-center px-5 py-3.5 border-b border-gray-50 last:border-0 hover:bg-gray-50/50 ${
                  activeTab === 'pendientes' ? 'grid-cols-[1fr_1fr_2fr_auto]' : 'grid-cols-[1fr_1fr_2fr_100px]'
                }`}>
                  ...
                  {/* Aprobados / Devueltos: badge de estado */}
                  {activeTab !== 'pendientes' && (
